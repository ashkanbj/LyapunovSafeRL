...
from utils.mpi_tools import (mpi_fork, mpi_statistics_scalar, mpi_sum,
                             num_procs, proc_id)
...
def get_target_update(main_name, target_name, polyak):
    ...
    for v_targ in targ_vars:
        assert v_targ.startswith(
            target_name
        ), f"bad var name {v_targ} for {target_name}"
...
def sac(
    ...
    x_ph, a_ph, x2_ph, r_ph, d_ph, c_ph = placeholders(
        obs_dim, act_dim, obs_dim, None, None, None
    )
    ...
    if fixed_entropy_bonus is None:
        with tf.variable_scope("entreg"):
            soft_alpha = tf.get_variable(
                "soft_alpha", initializer=0.0, trainable=True, dtype=tf.float32
            )
    ...
    if use_costs:
        if fixed_cost_penalty is None:
            with tf.variable_scope("costpen"):
                soft_beta = tf.get_variable(
                    "soft_beta", initializer=0.0, trainable=True, dtype=tf.float32
                )
    ...
    if proc_id() == 0:
        var_counts = tuple(
            count_vars(scope)
            for scope in ["main/pi", "main/qr1", "main/qr2", "main/qc", "main"]
        )
        print(
            (
    ...
    q_backup = tf.stop_gradient(
        r_ph + gamma * (1 - d_ph) * (min_q_pi_targ - alpha * logp_pi2)
    )
    ...
    def test_agent(n=10):
        for j in range(n):
            o, r, d, ep_ret, ep_cost, ep_len, ep_goals, = (
                test_env.reset(),
                0,
                False,
                0,
