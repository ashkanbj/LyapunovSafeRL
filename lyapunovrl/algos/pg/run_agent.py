...
import lyapunovrl.algos.pg.trust_region as tro
from lyapunovrl.algos.pg.agents import CPOAgent, PPOAgent, TRPOAgent
from lyapunovrl.algos.pg.buffer import CPOBuffer
from lyapunovrl.algos.pg.network import (count_vars, get_vars,
                                         mlp_actor_critic, placeholders,
                                         placeholders_from_spaces)
from lyapunovrl.algos.pg.utils import values_as_sorted_list
from lyapunovrl.utils.logx import EpochLogger
from lyapunovrl.utils.mpi_tf import MpiAdamOptimizer, sync_all_params
from lyapunovrl.utils.mpi_tools import mpi_fork, mpi_sum, num_procs, proc_id
...
def run_polopt_agent(
    ...
    seed += 10000 * proc_id()
    tf.set_random_seed(seed)  #   This is for tensorflow 1.x
    # tf.random.set_seed(seed)  #   This is for tensorflow 2.x
    np.random.seed(seed)
    ...
    if agent.learn_penalty:
        ...
        train_penalty = MpiAdamOptimizer(learning_rate=penalty_lr).minimize(
            penalty_loss
        )
    ...
    if agent.clipped_adv:
        min_adv = tf.where(
            adv_ph > 0, (1 + agent.clip_ratio) * adv_ph, (1 - agent.clip_ratio) * adv_ph
        )
    ...
    logger.setup_tf_saver(
        sess, inputs={"x": x_ph}, outputs={"pi": pi, "v": v, "vc": vc}
    )
