...
from lyapunovrl.algos.pg.utils import EPS
...
class TrustRegionAgent(Agent):
    def __init__(
        self, damping_coeff=0.1, backtrack_coeff=0.8, backtrack_iters=10, **kwargs
    ):
        super().__init__(**kwargs)
        self.damping_coeff = damping_coeff
        self.backtrack_coeff = backtrack_coeff
        self.backtrack_iters = backtrack_iters
        self.params.update(dict(trust_region=True))
...
class TRPOAgent(TrustRegionAgent):
    ...
    def update_pi(self, inputs):
        ...
        def set_and_eval(step):
            self.sess.run(
                set_pi_params, feed_dict={v_ph: old_params - alpha * x * step}
            )
...
class CPOAgent(TrustRegionAgent):
    ...
    def update_pi(self, inputs):
        ...
        for j in range(self.backtrack_iters):
            ...
            self.logger.log(
                "%d \tkl %.3f \tsurr_cost_new %.3f" % (j, kl, surr_cost_new), "blue"
            )
            if (
                kl <= target_kl
