...
class GridNavigationEnv(gym.Env):
    """ "
    Description:
        The environment is stochastic, sometimes the agent performs
        a random action, instead of the chosen one.
    ...
    Parameters for customize game:
        ...
        img_observation (bool):     whether to use an img_observation or
            a one-hot encoding of the player position, default: false

        Note: v0 has gridsize 25 and v1 has grisize 60 for
            further customization use the customize_game function

    Observation:
        this environment has two modes: the observation is either a
            one-hot encoding of the player position or an RGB image of the complete board
    ...
    def __init__(self, gridsize=32):
        ...
        self.FG_COLORS = {
            self.EMPTY_CHAR: (0, 0, 0),  # normal background
    ...
    def customize_game(
        self, seed=DEFAULT_SEED, rho=None, stochasticity=None, img_observation=None
    ):
        self.seed(seed)
        if not rho is None:
            self.rho = rho
    ...
    def make_game(self):
        ...
        self.art = self.build_asci_art(self.gridsize, self.rho).reshape(
        ...
        if self.img_observation:
            self.observation_space = spaces.Box(
                0, 1, shape=(self.gridsize, self.gridsize, 3), dtype=np.float32
            )
        else:
            self.observation_space = spaces.Box(
                0, 1, shape=(self.gridsize * self.gridsize,), dtype=np.float32
            )
    ...
    def step(self, action):
        ...
        if not (
            new_state_x < 0
            or new_state_x >= self.gridsize
            or new_state_y < 0
            or new_state_y >= self.gridsize
        ):
    ...
    def render(self, mode="human"):
        ...
        img = cv2.resize(
            img, (self.display_size, self.display_size), interpolation=cv2.INTER_NEAREST
        )
        if mode == "human":
            fig = plt.figure(0)
            plt.clf()
